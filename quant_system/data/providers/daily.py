...
class DailyDataProvider:
    """获取日线数据"""
    API_URL = "http://www.sanhulianghua.com:2008/v1/hsa_rixiandata"

    @log_exceptions(logger)
    def fetch(self, code: str, all_data: bool = False):
        """
        :param code: 股票代码
        :param all_data: True=全量历史数据，False=最新100笔
        """
        code = str(code).zfill(6)
        params = {
            'token': HSA_TOKEN,
            'code': code,
            'type': 'all' if all_data else '100'
        }
        ...
        df.attrs['ret'] = 200
        df.attrs['base'] = {'code': code, 'date': data.get('date', '')}
        return df
