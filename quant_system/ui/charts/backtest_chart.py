...
print("  [图表追踪] 进入 backtest_chart.py")
import os

print("  [图表追踪] 准备设置 QT_API 环境变量...")
os.environ["QT_API"] = "pyqt6"

print("  [图表追踪] 准备导入 matplotlib...")
import matplotlib

print("  [图表追踪] 准备修改 matplotlib backend 为 qtagg...")
matplotlib.use('qtagg')
print("  [图表追踪] matplotlib backend 修改完毕！")
...
print("  [图表追踪] backtest_chart 依赖全部导入完成。")

class BacktestChart(QWidget):
    """高级量化回测可视化图表 (三图联动版) - 纯净OO模式"""
    ...
    def init_ui(self):
        ...
        # 【核心修复1】彻底剔除 pyplot，使用原生的 OO API 设置全局样式
    ...
    def draw(self, df: pd.DataFrame, equity_df: pd.DataFrame, trades: list):
        ...
        try:
            ...
            # 【核心修复2】用面向对象的遍历法隐藏坐标轴文字，安全替代危险的 plt.setp()
