...
from quant_system.stock.threads import StockListThread
...
class StockListTab(QWidget):
    ...
    def initUI(self):
        ...
        self.cal_date_edit.setMaximumWidth(130)
        self.cal_date_edit.setDisplayFormat("yyyy-MM-dd")

        self.cal_check_btn = QPushButton("查是否为交易日")
        self.cal_check_btn.setMaximumWidth(130)
        self.cal_check_btn.clicked.connect(self.check_trading_day)

        tool_bar.addWidget(self.cache_btn)
        tool_bar.addWidget(self.update_btn)
        tool_bar.addStretch()
        tool_bar.addSpacing(10)
        tool_bar.addWidget(QLabel("日期:"))
        tool_bar.addWidget(self.cal_date_edit)
        tool_bar.addWidget(self.cal_check_btn)
        tool_bar.addSpacing(10)
    ...
    def check_trading_day(self):
        date = self.cal_date_edit.date().toString("yyyy-MM-dd")
        try:
            is_trade = self.dp.is_trading_day(date)
            if is_trade is None:
                QMessageBox.warning(self, "提示", f"{date} 查询失败")
            elif is_trade:
                QMessageBox.information(self, "提示", f"{date} 是交易日")
            else:
                QMessageBox.information(self, "提示", f"{date} 非交易日")
