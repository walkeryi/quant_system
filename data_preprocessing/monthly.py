# quant_system/data_preprocessing/monthly.py
import requests
import pandas as pd
...
class MonthlyDataProvider:
    """获取沪深个股月线数据"""
    API_URL = "http://www.sanhulianghua.com:2008/v1/hsa_yuexian"
    ...
    def fetch(self, code: str) -> pd.DataFrame | None:
        ...
        try:
            logger.info(f"正在向月线 API 请求数据: code={code}, all=0")
            resp = requests.get(self.API_URL, params=params, timeout=15)
            resp.raise_for_status()
            data = resp.json()

            if data.get('ret') != 200:
                logger.error(f"月线 API 错误 ({code}): {data.get('msg')} | 完整响应: {data}")
                return None

            records = data.get('data', [])
            if not records:
                # 【诊断级日志】：用来排查是不是服务端没给数据
                logger.warning(f"【服务端问题】月线接口成功响应(ret=200)，但并未返回100笔数据！服务端真实返回: {data}")
                return pd.DataFrame()
            ...
            logger.exception(f"月线数据处理发生意外异常 ({code}): {e}")
            return None