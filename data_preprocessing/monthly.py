...
class MonthlyDataProvider:
    ...
    def fetch(self, code: str) -> pd.DataFrame | None:
        ...
        try:
            logger.info(f"Fetching monthly data for {code} from API...")
            resp = requests.get(self.API_URL, params=params, timeout=15)
            resp.raise_for_status()
            data = resp.json()

            if data.get('ret') != 200:
                logger.error(f"Monthly API error for {code}: {data.get('msg')}")
                return None
            ...
            logger.exception(f"Unexpected monthly data error for {code}: {e}")
            return None