# -*- coding: utf-8 -*-
# 文件路径: quant_system/ui/sim_trade_tab.py
import json
import os
import time
...
from ui.threads.sim_trade_thread import SimTradeThread
...
class CreateTradeDialog(QDialog):
    ...
    def load_all_strategies(self):
        # 合并系统内置策略和用户自定义策略
        from ui.quant_backtest_tab import STRATEGY_CENTER_DATA
        all_strategies = {}
...
class SimTradeTab(QWidget):
    ...
    def initUI(self):
        ...
        self.btn_change = QPushButton("⚙️ 更换策略")
        ...
        # 3. 建议记录表格
        layout.addWidget(QLabel("📝 建议记录 (操作明细及系统日志)"))
        self.table_logs = QTableWidget(0, 4)
        self.table_logs.setHorizontalHeaderLabels(["时间", "动作", "价格", "成交量/说明"])
    ...
    def on_create_trade(self):
        if self._is_running():
            QMessageBox.warning(self, "警告", "当前已有模拟交易在运行，请先结束再创建新的交易！")
            return
    ...
    def on_change_strategy(self):
        ...
        # 复用创建交易的下拉逻辑，但禁用代码和资金的修改
        dialog = CreateTradeDialog(self)
        dialog.setWindowTitle("更换策略 (不可修改资金和标的)")
        ...
        if idx >= 0: dialog.combo_strategy.setCurrentIndex(idx)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            StateManager.save_sim_state(state) # 【解耦点】
            QMessageBox.information(self, "成功", "策略配置已热更新！引擎将使用新策略执行。")
            self.refresh_ui()
    ...
    def on_end_trade(self):
        ...
        reply = QMessageBox.question(self, "确认", "确定要结束本轮模拟交易吗？结束之后无法恢复。", 
    ...
    def show_history(self):
        history = StateManager.get_sim_history() # 【解耦点】
        if not history: return QMessageBox.information(self, "历史记录", "暂无历史交易记录。")
