...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.tabs.addTab(QWidget(), "消息中心")

        self.tabs.addTab(QWidget(), "模拟交易")
        ...
        try:
            ...
            if index == 1 and self.daily_tab is None:
                ...
                if self.chat_tab is None:
                    ...
                    self.tabs.insertTab(4, self.chat_tab, "消息中心")
                ...
                self.chat_tab.prepare_shared_link()



            elif index == 5:

                if self.sim_trade_tab is None:

                    from ui.sim_trade_tab import SimTradeTab

                    self.sim_trade_tab = SimTradeTab()

                    self.tabs.blockSignals(True)

                    self.tabs.removeTab(5)

                    self.tabs.insertTab(5, self.sim_trade_tab, "模拟交易")

                    self.tabs.setCurrentIndex(5)

                    self.tabs.blockSignals(False)

                self.sim_trade_tab.refresh_ui()
